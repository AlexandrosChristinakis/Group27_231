...
public class ImportanceMinHeap {
    ...
    // return true if A should be above B in a MIN-heap
    public boolean worse(Node A, Node B) {
        if (A.importance != B.importance)
            return A.importance < B.importance;  // lower importance = worse

        return A.word.compareTo(B.word) > 0;      // alphabetically larger = worse
    }
    ...
    private void heapifyDown(int pos) {
        while (true) {
            ...
            // Check left child
            if (leftChild <= size && worse(heap[leftChild], heap[smallest])) {
                smallest = leftChild;
            }

            // Check right child
            if (rightChild <= size && worse(heap[rightChild], heap[smallest])) {
                smallest = rightChild;
            }

            // If the "smallest" (worst) is not the current node, swap and continue
            if (smallest != pos) {
                swap(pos, smallest);
                pos = smallest;
            } else {
    ...
    public void heapSort() {
        ...
        for (int i = this.size; i > 1; i--) {
            // Move smallest element (root) to the end
            swap(1, i);

            // Shrink the heap size (i-th element now sorted)
            this.size--;
